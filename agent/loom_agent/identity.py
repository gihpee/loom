...
Wire format is kept in sync with src/loom/orchestrator/keys.py by contract, not
...
from loom_agent.hwinfo import gpu_fingerprint, sees_only_some_gpus

logger = logging.getLogger("loom_agent.identity")
...
def parse_join_key(key: str) -> JoinKey:
    ...
    if not key.startswith(KEY_PREFIX):
        raise BadJoinKey(
            "a join key starts with 'loom_'. Copy it whole from the admin page — "
