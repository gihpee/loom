...
from loom_agent.p2p.links import LinkTable
from loom_agent.p2p.peer import (
    DEFAULT_P2P_PORT,
    PeerNode,
...
def _enabled() -> bool:
    return os.environ.get("LOOM_P2P", "1").strip().lower() not in ("0", "false", "no")
    ...
    def on_rendezvous(self, addrs: List[str], relays: Optional[List[str]] = None) -> None:
        ...
        if not lattica_available():
            logger.info(
                ...
                "directly: pip install 'loom-agent[p2p]'"
            )
            return
