...
logger = logging.getLogger("loom_launcher.signature")

# Where the public half lives in the image. A file rather than a constant so
# anyone running their own Loom can ship their own key, and so it is obvious
...
def public_key_bytes() -> Optional[bytes]:
    ...
    override = os.environ.get("LOOM_RELEASE_PUBKEY", "").strip()
    if override:
