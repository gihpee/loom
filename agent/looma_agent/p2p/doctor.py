"""Ask one question and answer it: does the relay work from THIS machine?
    ...
    python -m looma_agent.p2p.doctor /ip4/203.0.113.7/tcp/47200/p2p/12D3KooW...
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="looma-p2p-doctor",
        description="check that a circuit-relay is usable from this machine",
    )
    parser.add_argument(
        "relay",
        nargs="?",
        default=os.environ.get("LOOMA_P2P_RELAY", ""),
        help="relay multiaddr (default: $LOOMA_P2P_RELAY)",
    )
    ...
    if not args.relay:
        print("no relay address given, and LOOMA_P2P_RELAY is empty.")
        print("Pass the multiaddr the relay printed at startup.")
        return 2
    try:
        from lattica import Lattica
    except Exception:
        print("the p2p stack is not installed here: pip install 'looma-worker[p2p]'")
        return 2
    ...
    node = (
        ...
        .with_key_path(tempfile.mkdtemp(prefix="looma-doctor-"))
        .with_dcutr(True)
        .with_autonat(True)
        .with_mdns(False)
        .with_upnp(False)
