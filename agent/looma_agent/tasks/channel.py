...
logger = logging.getLogger("looma_agent.tasks.channel")

TASK_HEADER = "X-Looma-Task"
RANK_HEADER = "X-Looma-To-Rank"
...
def deliver(port: int, payload: bytes, *, content_type: str = "application/octet-stream",
            from_rank: int = 0, path: str = "/looma/message") -> None:
    """Hand a message to a task listening on loopback."""
    request = urllib.request.Request(
        f"http://127.0.0.1:{port}{path}", data=payload, method="POST",
        headers={"Content-Type": content_type, "X-Looma-From-Rank": str(from_rank)},
    )
