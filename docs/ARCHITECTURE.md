...
- **`shard`-бэкенд** (`worker/loom_worker/shard/`) — единственный бэкенд,
  ...
  - `loader.py` материализует только свой диапазон слоёв (safetensors,
    ...
    embed/lm_head там, где они не нужны, и **скачивает только те файлы
    весов**, в которых лежат его тензоры: сначала метаданные, затем по
    `model.safetensors.index.json` — нужные `*.safetensors` (стадия из 20
    слоёв 14B-модели берёт около половины чекпойнта, а не весь);
...
- **`vllm_shard`-бэкенд** (`worker/loom_worker/vllm_stage/`) — та же стадия, но
  слои исполняет vLLM: paged KV-кэш, CUDA-графы, фьюженные ядра. Сервер стадии
  общий — движок подключается через тот же контракт исполнителя, поэтому
  транспорт, генерация и API не знают, что под ними. Механизм подробно разобран
  в [VLLM_PIPELINE.md](VLLM_PIPELINE.md).
...
## Хост с несколькими картами — одна нода

Воркер видит все GPU машины и сам раскладывает по ним слои своей стадии.

Раньше он предлагал только устройство 0: сообщал память одной карты, ставил на
неё стадию целиком, а остальные простаивали. Человек, подключивший машину с
четырьмя картами, отдавал одну.

**Почему делить внутри хоста, а не между хостами.** Граница между слоями на
двух картах — это копирование через PCIe, микросекунды на токен. Та же граница
между двумя машинами стоит сетевого перехода: замерено ~20 ms на токен. Слоёв
столько же, разница в тысячи раз.

Как это устроено:

- **Память суммируется по всем картам** — и свободная, и общая. Свободная, а не
  общая, по карте: карту, часть которой занята кем-то ещё, нода предлагает
  ровно на остаток.
- **Слои делятся пропорционально свободной памяти**, непрерывными кусками. 4090
  рядом с 3090 берёт бóльшую часть модели, а не половину. Непрерывность важна:
  k карт стоят k−1 переходов, а чередование стоило бы по переходу на слой.
- **Карта, на которой не помещается ни один слой, пропускается** — вместо того
  чтобы получить слой, который в неё не влезет.
- **Эмбеддинги живут с первым слоем, норма и голова — с последним**, чтобы
  скрытое состояние входило в стадию и выходило из неё без лишнего переноса.

Проверено на равенство: одни и те же слои на двух устройствах дают **те же
логиты и те же токены**, что и на одном. Где именно лежат слои — внутреннее
дело хоста, и на ответ это влиять не должно.

### Что осталось за пределами

`--gpu-memory-utilization` у vLLM — это доля **одной** карты, поэтому движкам
передаётся память в расчёте на карту, а не сумма по хосту. Иначе нода с двумя
картами по 24 ГБ заставила бы vLLM взять вдвое меньше положенного по квоте.

Сторож VRAM и так считал по всем устройствам — он остался без изменений.

### Ручное разделение машины

Карты можно отдать разным воркерам — например, если часть машины занята чем-то
своим:

```bash
docker run -d --gpus '"device=1"' --restart unless-stopped --network host \
  --name loomworker-1 gihpee/loomworker --key loom_<...>
```

Имя ноды при этом **подставляется само**. Раньше оба контейнера на одной
машине называли себя одинаково (с `--network host` у них общий hostname),
оркестратор считал их одной нодой, и каждая регистрация выбивала предыдущую:
нода мигала в таблице раз в секунду и ничего не обслуживала.

Теперь воркер, получивший **часть** карт машины, добавляет к hostname короткий
отпечаток доставшихся ему карт: `nv3-5fec`, `nv3-6b86`. Отпечаток считается по
UUID карт, а не по индексам — Docker перенумеровывает то, что пробрасывает, и
карта, переданная как `device=1`, внутри контейнера имеет индекс 0. UUID это
переживает, поэтому имя не меняется при перезапуске.

Воркер, которому досталась вся машина, оставляет чистый hostname — обновление
не превращает существующие ноды в новые.

Оркестратор со своей стороны замечает столкновение имён: если нода
регистрируется повторно через считанные секунды, в лог идёт предупреждение с
указанием причины, а не молчаливая замена сессии.

Явно заданный `LOOM_NODE_ID` всегда важнее автоматического. Явно указанное
устройство (`cuda:2`) не расширяется до всех карт.
