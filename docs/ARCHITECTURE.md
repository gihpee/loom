...
- **`shard`-бэкенд** (`worker/loom_worker/shard/`) — единственный бэкенд,
  ...
  - `executor.py` гоняет forward по своим слоям с per-request KV-кэшем. Сам
    проход делегирован модели transformers (`inner(inputs_embeds=...)`), а не
    написан вручную: маски внимания зависят от типа кэша, и своя реализация
    молча даёт другие числа. На GPU по умолчанию включены статический KV-кэш
    и `torch.compile` (CUDA-графы), активации между стадиями идут в bf16, а
    запросы обслуживаются параллельно (у каждого свой кэш; при включённых
    графах проходы сериализуются, потому что графы играют по фиксированным
    буферам);
