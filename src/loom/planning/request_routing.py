...
# Original: src/scheduling/request_routing.py — Phase-2 (chain selection):
# DynamicProgrammingRouting (DAG DP по τ<gi,ℓ> и ρ<gi,gi'>),
# RandomizedOverDynamicPipelinesRouting, RoundRobinOverFixedPipelinesRouting.
# Изменения: только переименованы импорты под неймспейс loom. Математика DAG DP
# не менялась. Источник живых данных (layer_latency_ms, rtt_to_nodes на Node)
# в Loom наполняется из централизованного Perf-map Store через
# loom/perfmap/sync.py вместо DHT-бродкастов Lattica; сам роутер этого не видит.
"""
...
from loom.planning.layer_allocation import BaseLayerAllocator
from loom.planning.node import Node
from loom.planning.node_management import NodeManager
