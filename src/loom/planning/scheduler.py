...
# Original: src/scheduling/scheduler.py — Scheduler (координация allocation,
# node lifecycle и request routing; event/dispatch loops).
# Изменения: только переименованы импорты под неймспейс loom. Логика не менялась.
# В Loom класс используется как программная библиотека: Scheduler Pool оркестратора
# создаёт по одному инстансу на модель (не синглтон процесса, без CLI).
"""
...
from loom.planning.node import Node, RequestSignal
