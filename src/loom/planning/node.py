...
# Original: src/scheduling/node.py — NodeHardwareInfo, RequestSignal,
# RooflinePerformanceModel, Node (serving state, capacity helpers, latency/RTT).
# Изменения:
#   1. Разорвана жёсткая связь Node <-> единственный ModelInfo для capacity:
#      Node.get_decoder_layer_capacity() и per_decoder_layer_kv_cache_memory
#      теперь делегируют в явный входной параметр `capacity: ShardCapacity`,
#      который вычисляет и передаёт Resource Broker (см. loom/planning/capacity.py).
#      Поле model_info сохранено ТОЛЬКО для roofline-оценки латентности и
#      num_layers (один инстанс планировщика по-прежнему обслуживает одну модель).
#   2. max_requests использует KV-бюджет из квоты (capacity.kv_cache_budget_bytes)
#      вместо полного объёма памяти устройства; зависимость от torch убрана
#      (elem_bytes берётся из model_info.cache_bytes_per_element напрямую).
#   Математика (roofline, латентности, water-filling входы) не менялась.
"""
...
from loom.planning.batching import compute_max_batch_size
...
class Node:
    ...
    - Capacity helpers for layer allocation (delegated to the explicit
      broker-provided `capacity` input);
    - Latency tracking and estimation if not available from node telemetry;
    ...
    hardware: NodeHardwareInfo
    model_info: ModelInfo
    # Explicit capacity input computed by the Resource Broker for this
    # (node, model) pair. Replaces the original in-Node capacity derivation.
    ...
    # Will be updated by worker telemetry via the Perf-map Store;
    ...
    @property
    def kvcache_mem_ratio(self) -> float:
        """KV-cache memory ratio (from the broker-provided capacity input)."""
        return self.capacity.kvcache_mem_ratio

    @property
    def param_mem_ratio(self) -> float:
        """Parameter memory ratio (from the broker-provided capacity input)."""
        return self.capacity.param_mem_ratio
    ...
    def max_requests(self) -> int:
        ...
        if self.start_layer is None or self.end_layer is None:
            return self.max_concurrent_requests
        elem_bytes = int(getattr(self.model_info, "cache_bytes_per_element", 2) or 2)
        derived_max = compute_max_batch_size(
            ...
            kv_cache_memory_fraction=self.capacity.kvcache_mem_ratio,
            ...
            kv_cache_budget_bytes=self.capacity.kv_cache_budget_bytes(),
    ...
    def get_decoder_layer_capacity(
        ...
        Delegates to the explicit broker-provided :class:`ShardCapacity` input
        instead of deriving capacity from a bound model.
        """
        return self.capacity.decoder_layer_capacity(
    ...
    def per_decoder_layer_kv_cache_memory(self) -> Optional[int]:
        ...
        return self.capacity.per_layer_kv_cache_memory(self.num_current_layers)
    ...
    def clear_serving_state(self) -> None:
        ...
        - Clear measured avg layer latency (will be re-learned / re-reported)
        """
