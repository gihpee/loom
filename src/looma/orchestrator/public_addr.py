...
into its code (`relay-lattica.gradient.network`); a self-hosted Looma needs its
own reachable entry point. This module finds it automatically, in order:

1. `LOOMA_PUBLIC_ADDR` — explicit override (a domain, a public IP, a VPN address).
    ...
    def note(self) -> Optional[str]:
        ...
        if self.address.startswith("127.") or self.address.startswith("localhost"):
            return (
                ...
                "машине. Задайте LOOMA_PUBLIC_ADDR (адрес в вашей сети или публичный) "
...
def _public_ip_via_service() -> Optional[str]:
    ...
    if os.environ.get("LOOMA_SKIP_IP_LOOKUP", "").lower() in ("1", "true", "yes"):
        return None
...
def resolve_public_address(grpc_port: int) -> PublicAddress:
    explicit = os.environ.get("LOOMA_PUBLIC_ADDR", "").strip()
    if explicit:
