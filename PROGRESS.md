...
- **Проход делегирован модели transformers.** Ручной цикл по слоям
  ...
  сохранён бит-в-бит.
- **Статический KV-кэш + `torch.compile(reduce-overhead)`** — CUDA-графы
  снимают per-layer накладные, которые на batch=1 и составляют основную
  потерю. Включено по умолчанию на GPU (`LOOM_STATIC_CACHE`, `LOOM_COMPILE`),
  на CPU выключено. Стадия пишет в лог, сколько памяти займёт кэш.
