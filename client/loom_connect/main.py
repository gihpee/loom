"""loom-connect — локальный порт до Ray-кластера в Loom.

    loom-connect 201.34.135.177:8000 group-cf6421224f --token <токен>
...
from loom_connect import __version__
from loom_connect.tunnel import Listener
from loom_connect.websocket import endpoint, opener
...
def parse(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument("--token", default=os.environ.get("LOOM_TOKEN", ""),
                        help="токен доступа; по умолчанию из LOOM_TOKEN")
...
async def run(args: argparse.Namespace) -> int:
    if not args.token.isascii():
        ...
        # X-Loom-Admin-Token header» — сообщением, которое называет заголовок,
