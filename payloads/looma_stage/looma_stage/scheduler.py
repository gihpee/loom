...
logger = logging.getLogger("looma_stage.scheduler")

# Сколько последовательностей держать одновременно.
#
# Это не потолок вкуса, а прямая цена в памяти: KV-кэш выделяется заранее и на
# всё обещанное число сразу. Для Qwen3-4B на половине слоёв один токен стоит
# 72 КБ, значит место под запрос длиной 4096 — 288 МБ, и шестьдесят четыре
# таких дают 18 ГБ кэша на КАЖДОЙ стадии.
#
# Шестнадцать, потому что узел чужой и на нём живёт не только эта задача. Кто
# держит карту целиком — поднимает число флагом и платит за него памятью
# осознанно, а не обнаруживает потом, что стадия крошечной модели заняла
# столько же, сколько огромная.
MAX_SEQUENCES = 16
